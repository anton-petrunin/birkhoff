...
\section{Affine theorems}

We discussed several theorems in Euclidean geometry that have affine nature;
that is, their assumptions and conculsions survive under affine transformations.
The examples in include 
\ref{thm:parallel-point-reflection},
\ref{lem:parallelogram}\ref{lem:parallelogram:midpoint},
\ref{ex:4parallels},
\ref{thm:centroid},
\ref{ex:midle}.
In this section, we present more examples of that type.

Suppose that $A$, $B$, and $X$ are distinct points on one line.
Note that 
\[\overrightarrow{AX}=\lambda\cdot \overrightarrow{BX},\]
where $\lambda=\pm\frac{AX}{BX}$.

Choose an affine transformation $P\mapsto P'$.
By the fundamental theorem of affine geometry (\ref{thm:fundamental-theorem-of-affine-geometry}), we have 
\[\overrightarrow{A'X'}=\lambda\cdot \overrightarrow{B'X'}.\]
...
Therefore this theorem belongs to affine geometry.
...
\begin{thm}{Exercise}\label{ex:ceva-affine}
Let $ABC$ be a nondegenerate triangle.
...
The last exercise is a partial case of \emph{Ceva's theorem}.
Note that the conclusions of Menelaus's and Ceva's theorem are identical.
These statements can be made more precise by taking the ratios $\frac{AC'}{BC'}$, $\frac{BA'}{CA'}$, and $\frac{CB'}{AB'}$ with sign.
Namely, we assume that signed ratio $(\frac{AX}{BX})=\pm \frac{AX}{BX}$ meets the equation
\[\overrightarrow{AX}=\left(\frac{AX}{BX}\right)\cdot \overrightarrow{BX}.\]

With the signed ratios, the conclusions of the two theorems look different

...
soon or later you will have to learn the Lebesgue measure if it is not done already.
...
The Lebesgue measure provides an example of an area function; 
...
Any degenerate polygonal set has a vanishing area.
\end{thm}
...
Further \index{63@$\lfloor x \rfloor$, $\lceil x \rceil$}$\lfloor x \rfloor$ and $\lceil x \rceil$ will denote \index{floor and ceiling}\emph{floor} and \emph{ceiling} of $x$;
...
In this section, we will give examples of slim proofs using the properties of the area function.
...
\begin{thm*}{Lemma}
If $\triangle A B C$ is nondegenerate and its angle bisector at $A$ intersects $[BC]$ at point~$D$.
Then 
...
changing $\mathcal{U}$ will require the conversion of area units.
...
is taken so that on small scales area behaves like in the Euclidean plane.
Say, 
...
Recall that \index{defect}\emph{defect of triangle} $\triangle ABC$ is defined as 
$$\defect(\triangle ABC)
\df 
...
Since the angles of an ideal triangle vanish, any ideal triangle in h-plane has area~$\pi$.
Similarly, in the unit sphere, the area of an equilateral triangle with right angles has area $\tfrac\pi2$;

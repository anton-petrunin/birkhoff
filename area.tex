...
The so called {}\emph{Lebesgue measure} gives an example of such function.
In particular a construction of Lebesgue measure implies the existence of area function.
...
A class of sets that closed with respect to union and intersection  is called a {}\emph{ring of sets}.
...
Lebesgue measure, provides an example of area function; 
namely if one takes $\area\mathcal{P}$  to be Lebesgue measure of $\mathcal{P}$,
then the function $\mathcal{P}\mapsto\area\mathcal{P}$ satisfies the above conditions.

The construction of Lebesgue measure can be found in any textbook on real analysis.
We do not discuss it here.

If the reader is not familiar with Lebesgue measure, then he should take existence of area function as granted; 
...
Applying this formula few times we get the general case.
...
\parbf{Lemma ~\ref{lem:bisect-ratio}.}
\textit{If $\triangle A B C$ is nondegenerate and its angle bisector at $A$ intersects $[BC]$ at the point~$D$.
Then 
...
The second statement in the following exercise is called \index{Ceva's theor}\emph{Ceva's theorem}.
...
The set $\mathcal{U}$ in this case plays role of the unit measure for the area
...
the area function satisfying the conditions on page \pageref{page:area-def} is uniquely defined.

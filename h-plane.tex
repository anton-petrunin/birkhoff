...
In this chapter, we study the geometry of the plane described by the conformal disc model, the h-plane for short.
...
The value $\phi$ is called the \index{angle!angle of parallelism}\emph{angle of parallelism} at $P$ to~$\ell$.
...
the remaining parallel lines are called \index{parallel lines!ultraparallel lines}\emph{ultraparallel}.


In the picture, the two solid h-lines passing thru $P$ are asymptotically parallel to~$\ell$;
the dashed h-line is ultraparallel to~$\ell$.
...
Let $Q$ be the footpoint of $P$ on an h-line~$\ell$.
Then
...
\end{align*}
Taking the exponential of both sides and simplifying, we get the second identity.
...
\section{Inradius of an h-triangle}
...
Such a curve $g$ is called an \index{equidistant}\emph{equidistant} to the h-line~$m$.%
\footnote{It can also be called a \index{hypercycle}\emph{hypercycle} with central line $m$.}
...
Since any three points lie on a circline, we have that any nondegenerate h-triangle is inscribed in an h-circle, a horocycle, or an equidistant.
...
Recall that \index{angle-preserving transformation}\emph{angle-preserving transformation} is a bijection from an h-plane to itself such that 
...
Suppose $P\ne O$; let $Z$ denote the inverse of $P$ across the absolute.
...
\section{The Pythagorean theorem}\index{Pythagorean theorem}
...
Assume that the h-triangle $ACB$ has a right angle at~$C$.
Then
...
For large $a$ and $b$ the terms $e^{-a}$, $e^{-b}$, and $e^{-a-b+\ln 2}$ are negligible.

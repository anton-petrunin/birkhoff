...
We may also use the axioms of neutral geometry since they all hold in the h-plane; the latter is proved in the previous chapter.
...
Show that $|\measuredangle_h CDA|>\phi$, where $\phi$ is the angle of parallelism of $D$ to~$(AB)_h$.
\end{thm}
...
Let $Q$ be the footpoint of $P$ on h-line~$\ell$.
Then
...
\begin{thm}{Exercise}\label{ex:right-angle-parallelism} Let $\triangle_hABC$ be an h-triangle with right h-angle at $C$.
...
If you turn the wheel further a bit, you start to run along a path that stays at a fixed distance from an h-line.
...
\index{equidistant}\emph{equidistant} to h-line~$m$.\footnote{It can be also called \index{hypercycle}\emph{hypercycle}.}
...
here $A$, $B$ are h-points distinct from the center of absolute and $A'$, $B'$ are their inversions across the absolute.

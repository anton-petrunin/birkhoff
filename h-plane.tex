...
Sine the angle $PZB$ is shared,  $\triangle ZBX\sim \triangle ZPB$.
In particular, 
$$ZX\cdot ZP=ZB^2;$$
...
Consider arbitrary h-triangle $XYZ$.
...
of the ideal triangle is the center of the absolute. 
...
Consider the h-plane with absolute~$\Omega$.
...
So if $P',P\in g$ are inversions of each other in $\Delta$, then they lie on the same h-distance from~$m$.
...
\parit{Proof.}
Consider an h-triangle $ABC$.
According to Theorem~\ref{thm:3sum-a},
$$\defect(\triangle_hABC)\ge 0.\eqlbl{eq:defect<0}$$
It remains to show that in the case of equality the h-triangle $ABC$ degenerates.
...
Consider the h-plane with absolute formed by the unit circle centered at~$O$.
...
In this case the h-distance is the minimal time needed to travel from one point of the h-plane to the other point.

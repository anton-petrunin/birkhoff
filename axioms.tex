...
Our system is based on the \emph{key observations}  (\ref{preaxiomI})--(\ref{preaxiomV}) listed on page~\pageref{preaxiomI}.
...
Note that our system is build upon metric spaces.
...
it reminds a model-based introduction to Euclidean geometry.
...
\let\thefootnote\relax\footnotetext{${}^\a$ A statement marked with ``$\a$'' if Axiom~\ref{def:birkhoff-axioms:4} was not used in its proof. Ignor this mark for a while; it will be important in Chapter~\ref{chap:non-euclid}, see page \pageref{a-mark}.}

...
around 300 BC --- the most successful and influential textbook ever written.
...
This branch of mathematics is called ``Foundations of geometry''.
...
It contains twenty axioms in five groups, six ``primitive notions'' three ``primitive terms'', these are not defined in terms of previously defined concepts.

We will use an other system of axioms,
which is very close to the system proposed by George Birkhoff in 1932.
This choice is made to minimize the tedious parts
which are unavoidable in the more conservative Hilbert's approach.

This system is build upon metric spaces.
In particular we use the real numbers as a building block. By that reason this approach reminds a model-based introduction to Euclidean geometry.
...
angles,
triangles,
as well as the relations ``$\equiv$'' and ``$\cong$''.

...
This system is based on the {}\emph{key observations}  (\ref{preaxiomI})--(\ref{preaxiomV}) listed in Section~\ref{preaxioms}.
...
Ignore this mark for a while; it will be important in Chapter~\ref{chap:non-euclid}.}
...
\parit{Proof.}
According to the definition of half-line, 
...
We say that the point $O$ \index{between}\emph{lies between} points $A$ and $B$, 
if $O\not= A$, $O\not= B$, and $O\in[A B]$.

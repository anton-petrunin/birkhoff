...
If a statement above marked with ``$\a$'',\label{a-mark} for example ``\textbf{Theorem.\abs}'', then it holds in any neutral plane and the same proof works.
...
Prove the \index{SAA congruence condition}\emph{side-angle-angle congruence condition} holds in the neutral geometry.
...
It is hard to imagine a neutral plane, which does not satisfy some of the properties above.

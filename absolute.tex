...
We use Axiom~\ref{def:birkhoff-axioms:4} starting from Chapter~\ref{chap:parallel},
and all the statements before that hold in neutral geometry.
...
Recall that a statement is marked with ``$\a$'' (for example, ``\textbf{Theorem\abs}'') if it holds in every neutral plane, and the same proof works.
...
Read the proof of Theorem~\ref{thm:tangent-angle} and identify the first statement that does not work in the neutral plane.
\end{thm}
...
which in turn use Theorem~\ref{thm:3sum}.
...
The \index{defect}\emph{defect} of triangle $\triangle ABC$ is defined as
$$\defect(\triangle ABC)
\df 
...
Simply exchange Axiom~\ref{def:birkhoff-axioms:4} for Axiom~h-$\!$\ref{def:birkhoff-axioms:4}
 and start to prove theorems in the obtained axiomatic system.
If we arrive at a contradiction,
we prove Axiom~\ref{def:birkhoff-axioms:4} in a neutral plane.
This idea had been growing since the $5^\text{th}$ century;
...
the more believable it became that there is no contradiction;
...
It seems that Lobachevsky was the first who had a proof of Theorem~\ref{thm:consistent} altho its formulation required rigorous axiomatics which was not developed in his time.

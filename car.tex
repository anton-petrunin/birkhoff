...
The points $A_1,\dots, A_n$ are \index{vertex!of a regular $n$-gon}\emph{vertexes},
...
We say that one set of tools is \emph{stronger} than another if any point that can be constructed with the second set can be constructed with the first set alone.
If it holds the other way as well, then we say that the two sets of tools are \emph{equivalent}. 

For example \emph{Mohr--Mascheroni theorem} states that compass alone is equivalent to compass and ruler.

To prove Mohr--Mascheroni theorem, 
it is sufficient to perform the following two constructions using compass alone:
(1) given four points $X$, $Y$, $P$ and $Q$, construct the intersection of the lines $(XY)$ and $(PQ)$
and (2) given four points $X$, $Y$, and a circle $\Gamma$, construct the intersection of the lines $(XY)$ and $\Gamma$.
Indeed, once it is done all the steps in a construction with compass and ruler can be performed with compass alone.

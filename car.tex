...
For example, assume we want to find the points $X_1=(x_1,y_1)$ and $X_2\z=(x_2,y_2)$ at the intersection of
a line passing thru $A=(x_A,y_A)$ and $B\z=(x_B,y_B)$
with the circle with center $O=(x_O,y_O)$ that passes thru the point $W=(x_W,y_W)$.
...
Given two line segments with lengths $a$ and $b$, provide a ruler-and-compass construction of segments with lengths $\tfrac {a^2}b$ and $\sqrt{a\cdot b}$.
\end{thm}
...
with a given line, and it can also be used as a ruler.
...
with a given line and can be used as a ruler.
...
A regular 7-gon provides another example of that type --- it is easy to verify, while the Gauss--Wantzel theorem states that it is impossible to construct with a ruler and compass.
...
It is based on Exercise~\ref{ex:equilateral triangle}, which states that it is impossible to construct an equilateral triangle with a set-square only.
...
This observation leads to a source of impossible constructions in a stronger sense --- those that are not even verifiable.
...
Show that with a circumtool and an inversor,

...
\begin{itemize}
\item If $|\measuredangle A O B|=\tfrac\pi2$, we say that $\angle A O B$ is \index{right!angle}\emph{right};
...
\item If $|\measuredangle A O B|>\tfrac\pi2$, we say that $\angle A O B$ is \index{angle!right, acute, and obtuse angles}\index{obtuse}\emph{obtuse}.
\end{itemize}
...
If $\angle A O B$ is right, we may also say that $[O A)$ is \index{perpendicular}\emph{perpendicular} to $[O B)$.
...
The line $\ell$ thru $M$ and perpendicular to $(AB)$ is called the \index{bisector!perpendicular bisector}\index{perpendicular!bisector}\emph{perpendicular bisector} of the segment~$[AB]$.
...
Let $\ell$ be a perpendicular bisector of $[A B]$, and $X$ be an arbitrary point in the plane.
Show that 
$AX<BX$ if and only if $X$ and $A$ lie on the same side of~$\ell$.
\end{thm}
...
From above we can choose a point $P'$ in such a way that $\ell$ is the perpendicular bisector of~$[PP']$.
...
Show that $\measuredangle XBY\equiv 2\cdot \measuredangle ABC$.
...
\begin{thm}[!]{Classroom exercise}\label{ex:3-reflections}
...
\begin{thm}{Classroom exercise}\label{ex:right-acute}
Show that
\begin{enumerate}[(a)]
...
If a triangle has an obtuse or right angle, then it is called \index{triangle!right, acute, and obtuse triangle}\index{obtuse}\emph{obtuse} or \index{triangle!right triangle}\index{right!triangle}\emph{right}, respectively.
Otherwise, if all its angles are acute, it is called an \index{acute}\emph{acute triangle}.
...
Recall that a circle with radius $r$ and center $O$ is the set of all points at distance $r$ from $O$.
We say that a point $P$ lies \index{inside!a circle}\emph{inside} the circle if $OP<r$;
...
Assume a line $\ell$ passes thru the point $P$
...
Show that the line between the centers of $\Gamma$ and $\Gamma'$ is a perpendicular bisector of~$[A B]$.
\end{thm}

\begin{thm}{Exercise}\label{ex:center}
Describe a ruler-and-compass construction of the center
of a given circle.
\end{thm}
...
a line $\ell$ and a circle $\Gamma$ might have 2, 1, or 0 points of intersection.
...
two distinct circles might have 2, 1, or 0 points of intersection.
If $P$ is the only point of intersection of circles $\Gamma$ and $\Gamma'$,
we say that \index{tangent!circles}\emph{$\Gamma$ is tangent to $\Gamma'$ at $P$}; we also assume that a circle is tangent to itself at each of its points.
...
\begin{enumerate}[(a)]
\item\label{ex:tangent-circles:a} The circles meet at a point~$P$ that lies on the same line with $O$ and~$O'$.
...
Prove that their centers lie on the same line.
\end{thm}
...
describe a ruler-and-compass construction of a circle with radius $AB$

...
\index{angle!acute angle}\index{acute!acute angle}\emph{acute};
\item If $|\measuredangle A O B|>\tfrac\pi2$, we say that the angle $\angle A O B$ is  \index{angle!obtuse angle}\index{obtuse!obtuse angle}\emph{obtuse}.
\end{itemize}
...
The line $\ell$ passing through $M$ and perpendicular to $(AB)$
 is called \index{perpendicular bisector}\emph{perpendicular bisector} to the segment $[AB]$. 
...
or line $(BX)$ is the \index{bisector}\emph{bisector} of $\angle ABC$.
...
\begin{wrapfigure}[6]{o}{39mm}
\begin{lpic}[t(-5mm),b(0mm),r(0mm),l(1mm)]{pics/bisectors(1)}
\lbl[tr]{30,5;$A$}
\lbl[rb]{9,10;$B$}
\lbl[br]{31,29;$C$}
...
the set $\Gamma$ of all points on distant $r$ from $O$ is called \index{circle}\emph{circle} 
...
A chord passing through the center is called \index{diameter}\emph{diameter}.
...
we say that \index{tangent line}\emph{$\ell$ is tangent to $\Gamma$ at $P$}. 
...
The idealized compass can be used only to draw a \index{circle}\emph{circle} with given center and radius.

...
it follows that two lines $(O A)$ and $(O B)$ can be called {}\emph{perpendicular} if $[O A)\z\perp [O B)$.
In this case, we also write $(O A)\z\perp (O B)$.



\begin{thm}{Exercise}\label{ex:acute-obtuce}
Assume that a point $O$ lies between $A$ and $B$ and $X\ne O$.
Show that 
...
Let $\ell$ be a perpendicular bisector to $[A B]$, and $X$ be an arbitrary point on the plane.
...
and composition of direct and indirect motions is indirect.

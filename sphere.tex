...
The \index{distance!spherical}\index{spherical distance}\emph{spherical distance} between \( A \) and \( B \) (denoted briefly as \( AB_s \)) is defined as the absolute value of the angle measure \( \measuredangle AOB \), measured in the plane containing \( O \), \( A \), and \( B \).

Note that each great circle is a circle of radius \( \tfrac{\pi}{2} \) on $\Sigma$.
...
A solid spherical triangle \( \solidtriangle_s ABC \) is defined as the set that includes the sides of \( \triangle_s ABC \) along with the region inside;
...
For every solid spherical triangle $\Delta$, we have
...
According to Section~\ref{The defect}, \textit{the defect of every triangle in a neutral plane is nonnegative}.


\begin{thm}{Exercise}\label{ex:defect-sphere}
Show that the defect of every spherical triangle is nonpositive.
\end{thm}
...
Let us give a couple of hints to the reader who wants to reconstruct its proof.
...
a sphere 
if and only if
for every plane $\Pi$ passing thru $O$,
...
This is not a coincidence; the same holds for all analytic formulas: changing every distance $d$ to $i\cdot d$ transforms a valid spherical formula into a valid hyperbolic formula (the angle measures must remain the same).

...
Make a ruler-and-compass construction of the inversion of a given point in a given circle.
\end{thm}
...
Let $\square ABCD$ and $\square A'B'C'D'$  be two quadrilaterals
...
and we will denote it as~\index{1inf@$\infty$}$\infty$.
...
\parit{Proof.}
Denote by $O$ the center of the inverse.
...
Since $\Gamma$ is the inversion of $\Gamma'$ it is sufficient to prove that 
$$\infty\in \Gamma
\quad
\iff
\quad
...
Assume that the circle $\Gamma'$ 
is the inversion of the circle $\Gamma$.
...
Let $\square ABCD$ be an inscribed quadrilateral.
...
It remains to consider the case when the quadrilateral $\square ABCD$ is inscribed in a circle, say~$\Gamma$. 
...
Consider three circles tangent to each other and to two parallel lines as shown on the picture;
...
Denote by $\Gamma'$ the inversion of~$\Gamma$.
Since $A$ is the inversion of itself, the points $A$, $P$ and $P'$ lie on~$\Gamma'$.
...
\parit{Proof.}
Let $P'$ be the inversion of the point $P$ in the circle~$\Omega$.
...
and a tool  which constructs an inversion of a given point in a given circle.
...
The latter follows from Theorem~\ref{lem:inverse-4-angle}(\ref{lem:inverse-4-angle:angle}).
\qeds
...
Assume $P$ is the inversion of $Q$ in $\Gamma$, 
then $P'$ is the inversion of $Q'$ in~$\Gamma'$.
\end{thm}

\parit{Proof.}
If $P=Q$, then $P'=Q'\in\Gamma'$. 
Therefore, $P'$ is the inversion of $Q'$ in~$\Gamma'$.

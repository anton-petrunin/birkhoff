...
Assume that $n\perp \ell$ and $n\perp m$.
Then $\ell\parallel m$.
\end{thm}

\parit{Proof.}
Assume the contrary; 
that is, $\ell$ and~$m$ meet at one point, say $Z$.
Then by Theorem~\ref{perp:ex+un},
$\ell=m$.
Since every line is parallel to itself, we have that $\ell\parallel m$ --- a contradiction.
\qeds
...
and second to construct the line $m$ thru $P$ that is perpendicular to~$n$.
...
The following exercise provides a partial converse to Proposition \ref{prop:perp-perp}.

\begin{thm}[!]{Exercise}\label{ex:perp-perp}
Let $k$, $\ell$, $m$, and $n$ be lines.
\begin{enumerate}[(a)]
 \item\label{ex:perp-perp:a}
 Assume $\ell\parallel m$ and $m\perp n$.
 Show that $\ell\perp n$.
 \item\label{ex:perp-perp:b}
 Assume $k\perp \ell$, $\ell\parallel m$, and $m\perp n$.
 Show that $k\parallel n$.
\end{enumerate}
...
Denote by $Z'$ the reflection of $Z$ across $O$.
...
Let $A$, $B$, $C$, and $D$ be points such that $A\ne B$, $B\ne C$, and $C\ne D$.
Then $(AB)\parallel(C D)$ if and only if
...
\begin{thm}{Exercise}\label{ex:parallel-angles}
...
Any parallelogram is \index{central symmetry}\emph{centrally symmetric} with respect to the midpoint of one of its diagonals;
...
Assume that point $P$ is distinct from the vertices of a parallelogram $ABCD$.
...
Conclude that a line can be defined as a subset of the coordinate plane of any of the following types:
\begin{enumerate}[(a)]
\item  Solutions of an equation $a\cdot x+b\cdot y=c$
for constants $a$, $b$, and $c$ such that $a\ne 0$ or $b\ne0$.
\item\label{ex:line-coord:parameter} The set of points $(a\cdot t+c,b\cdot t+d)$ for all $t\in \mathbb{R}$, where  $a$, $b$, $c$, $d$ are constants with $a\ne 0$ or $b\ne0$.
\end{enumerate}

...
\index{triangle!similar triangles}\index{similar triangles}\emph{similar} (briefly \index{1rel@$\sim$}$\triangle A' B' C'\z\sim\triangle A B C$) if their sides are proportional; 
that is, 
$$A' B'
=
k\cdot A B,
...
Each of these conditions is proved by applying Axiom~\ref{def:birkhoff-axioms:4} with SAS, ASA and SSS congruence conditions correspondingly
...
Let $\triangle A B C$ be an acute triangle. 
...
In any triangle $\triangle A B C$, we have
...
A quadrilateral formed by quadruple $(A,B,C,D)$ will be denoted by \index{1gon@$\square$}$\square ABCD$.
...
Show that for any quadrilateral $\square ABCD$, we have
...
A nondegenerate quadrilateral $\square ABCD$ is called a \index{parallelogram}\emph{parallelogram}
...
A quadraliteral $\square ABCD$ is called a \index{rectangle}\emph{rectangle} if the angles $\angle ABC$, $\angle BCD$, $\angle CDA$ and $\angle DAB$ are right.
...
Show that the parallelogram $\square ABCD$ is a rectangle
if and only if $AC=BD$.
\end{thm}

\begin{thm}{Exercise}\label{ex:romb2}
Show that the parallelogram $\square ABCD$ is a rhombus

...
Finally, the reflection across $O$ can not be indirect since $\measuredangle XOY\z=\measuredangle X'OY'$;
...
Note that 
\[\measuredangle ABO=\measuredangle ABC,
\qquad
\measuredangle A'BO=-\measuredangle BCA'.\eqlbl{eq:A B O= A B C}
\]
Since $A'$, $C$ and $D$ lie on one line, Exercise~\ref{ex:ABCO-line} implis that 
\[2\cdot BCD\equiv 2\cdot BCA'.\eqlbl{eq:2BCD= 2BCA'}\]
...
By transversal property (\ref{thm:parallel-2}), we get that $(AX)\parallel (CB)$.

Since $[BM]$ and $[MX]$ do not intersect $(CA)$,
the points $B$, $M$, and $X$ lie on the same side of $(CA)$.
Therefore, applying the transversal property  for transversal $(CA)$, we get that 

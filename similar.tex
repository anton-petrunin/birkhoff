...
These 4 points are called \index{vertex!of quadrangle}\emph{vertexes} of quadrangle.
...
the remaining two segments $[AC]$ and $[BD]$ are called \index{diagonals of quadrangle}\emph{diagonals of $\square ABCD$}.
...
Using the proof above together with Corollary~\ref{cor:inscribed-quadrangle}, one can show that the equality holds only if the vertexes $A$, $B$, $C$, and $D$ appear on a line or a circle in the same cyclic order;

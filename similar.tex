...
These 4 points are called \index{vertex!of quadrilateral}\emph{vertexes} of quadrilateral.
The quadrilateral $ABCD$ will be also denoted by \index{25@$\square$}$\square ABCD$.

Given a quadrilateral $ABCD$,
the four segments $[AB]$, $[BC]$, $[CD]$, and $[DA]$ are called \index{side!of quadrilateral}\emph{sides of $\square ABCD$};
...
Using the proof above together with Corollary~\ref{cor:inscribed-quadrilateral}, one can show that the equality holds only if the vertexes $A$, $B$, $C$, and $D$ appear on a line or a circle in the same cyclic order;

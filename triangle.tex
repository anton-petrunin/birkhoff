...
since $\ell\perp(AB)$, we get that
$m\perp(AB)$ (see Exercise~\ref{ex:perp-perp}).
But $m\perp (AC)$, and by Theorem~\ref{perp:ex+un} we have $(AC)=(AB)$;
...
Since $(B' C')\parallel (BC)$, Exercise~\ref{ex:perp-perp} implies that the altitude from $A$ is perpendicular to $[B' C']$, and from above, it bisects~$[B' C']$.

...
Assume $H$ is the orthocenter of an acute (or obtuse) triangle $A B C$.
Show that $A$ is the orthocenter of $\triangle H B C$.
\end{thm}

$H=C$ if and only if $\triangle A B C$ has a right angle at $C$.
This justifies the acute (or obtuse) condition in the exercise.
...
Assume that a bisector at $A$ of a nondegenerate triangle $ABC$ intersects $[BC]$ at $D$;
...
A rhombus $AXYZ$ is inscribed in a nondegenerate triangle $ABC$ so that $X\in [AB]$, $Y\in [BC]$, and $Z\in[CA]$.
Show that the side of the rhombus is $\sqrt{XB\cdot ZC}$.
\end{thm}

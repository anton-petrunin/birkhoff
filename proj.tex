...
\emph{central pensil} containing all lines passing thru a fixed point called the \index{center!center of the pencil}\emph{center of the pencil}
and  
\emph{parallel pensil} containing pairwise parallel lines.
...
Then all the points $P$, $Q$, $R$, $P'$, $Q'$, $R'$ lie in $\Theta$, .
...
An inversion is not defied at its center;
morover, the center is not an inverse of any point.
...
Show that Axiom p-\ref{def:proj-axioms:3}$\,'$ is equivalent to Axiom p-\ref{def:proj-axioms:3}.

That is, p-\ref{def:proj-axioms:1}, p-\ref{def:proj-axioms:2} and p-\ref{def:proj-axioms:3} imply p-\ref{def:proj-axioms:3}$\,'$, 
and p-\ref{def:proj-axioms:1}, p-\ref{def:proj-axioms:2} and \ref{def:proj-axioms:3}$\,'$ imply p-\ref{def:proj-axioms:3}.
\end{thm}
...
\[n^2+n+1.\]
\item\label{ex:oder:c} Show that that there is no projective plane with exactly 10 points.

...
This incidence structure is studied in projective geometry.

A parallel pencil contains the ideal line and the lines $y=m\cdot x+b$ with fixed slope $m$;
if $m=\infty$, we assume that the line is given by an equation $x=a$.
...
Let us identify the Euclidean plane plane with a plane $\Pi$ in the Euclidean space $\mathbb{R}^3$ that does not pass thru the origin $O$.
...
It follows since the coordinates have to be proportional;
points on $\Pi$ have unit $z$-coordinate, 
and points on $\Pi'$ have unit $y$-coordinate.

The perspective projection, maps one plane to another.
However, we can identify the two planes, say by fixing a coordinate system in each.
...
The same strategy worked for perspective projection $\Pi\to\Pi'$, but this time real projective plane is the right choice of extension.}
...
\end{itemize}
This way we extend the perspective projection to a bijection from real projective space to itself.
...
If $\gamma$ sends the ideal line to the line $\ell$, choose a perspective projection $\beta$ as above.
...
That is, the bijections $P\leftrightarrow \dot P$ and $\ell\leftrightarrow \dot \ell$
...
The existence of duality in the real projective plane makes possible to formulate an equivalent dual statement to any statement in projective geometry.

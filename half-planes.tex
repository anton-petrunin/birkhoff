...
Assume that for any $t\z\in [0,1]$ we have three points $O_t$, $A_t$, and $B_t$ in the plane such that
\begin{enumerate}[(a)]
...
Given $\beta\in [0,\pi]$,
suppose that $C_\beta$ denotes the point such that $BC_\beta\z=a$ and $\measuredangle ABC_\beta=\beta$.
...
the values
$\beta$ and~$r$ determine $\triangle ABC$ up to congruence.
In particular, the distance $AC$ depends only on $\beta$ and~$r$;
thus, we can consider the function $s$ defined by
$$s(r,\beta)\df AC.$$

...
$$f\:[P Q)\to [0,\infty)$$
such that $f(P)=0$.
By the definition of an isometry, $P X=f(X)$ for every $X\z\in [P Q)$.
Thus, $P X=r$ if and only if $f(X)=r$.
...
Check the definition for $\delta=\epsilon$.
...
Apply \ref{cor:half-plane}\textit{\ref{cor:half-plane:intersect}} to show that that $O$ and $A'$
...
Assume $\ell\perp n$.
Apply \ref{thm:parallel} to show that $m\nparallel n$, so $m$ and $n$ meet at one point, say~$P$.
Draw a line $m'\perp n$ thru $P$.
By \ref{prop:perp-perp}, $m'\parallel \ell$.
Apply \ref{thm:parallel} to show that $m= m'$.

Repeat the argument assuming that $m\perp n$.
...
Furhter, show that the composition of two indirect motions is direct and use that reflection across a line is indirect (see \ref{cor:reflection+angle}).

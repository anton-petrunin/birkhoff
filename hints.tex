...
\raggedcolumns\setlength{\multicolsep}{-8mm}
...
(Alternatively, use \ref{thm:3sum-a} to show that $|\measuredangle CBD|\ge|\measuredangle ADB|$ and then use this to show that $c\ge a$.)
...
\footnote{We could also move $Q$ to the center of absolute.
...
By \ref{lem:P-->O}, we can assume that $O$ is the h-midpoint of $L$ and $M$.
...
Note that the angle of parallelism of $B$ to $(CD)_h$ is greater than $\tfrac\pi4$,
...
\parbf{\ref{ex:f(1)=1}.} Set $a=f(1)$ and $b=f(0)$.
...
Let $\iota_\Gamma$ and $\iota_\Omega$ be the inversions across $\Gamma$ and~$\Omega$.

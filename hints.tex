...
Further, any line that connects a pair of points on these three lines $(AB)$, $(AX)$, and $(BX)$ is mapped to~$(A'B')$.

Note that any point $Z$ lies on a line passing thru two distinct points on these three lines.
Therefore, $Z'\z\in(A'B')$; that is, the whole plane is mapped to $(A'B')$.
...
Hence the bijection maps lines to lines, and the second statement follows.
...
Apply an affine transformation that changes the angle between the lines;
it exists by Theorem~\ref{thm:fundamental-theorem-of-affine-geometry}.
...
Fix a coordinate system and apply Corollary~\ref{cor:affine-linear} for $O=(0,0)$, $X=(1,0)$ and $Y=(0,1)$.

...
\parbf{\ref{ex:dist-square}.} Check the triangle inequality for $A=0$, $B=1$ and $C=2$.
...
It remains to apply \ref{ex:2.4} and Theorem~\ref{thm:straight-angle} correspondingly in these two cases.
...
Apply \ref{ex:pbisec-side}, Theorem~\ref{thm:ASA}, and \ref{ex:PP(PN)}.
...
\parbf{\ref{ex:chord-perp}.} Apply Theorem~\ref{thm:perp-bisect}.

\parbf{\ref{ex:two-circ}.} Use \ref{ex:chord-perp} and Theorem~\ref{perp:ex+un}.
...
\parbf{\ref{ex:coordinates}.} \textit{(a).} Use the uniqueness of the parallel line (Theorem~\ref{thm:parallel}).
...
\parbf{\ref{ex:bisector-parallel}.} Apply \ref{thm:isos}, \ref{thm:parallel-2} 
...
\textit{(Note that in the Euclidean plane the statement follows from Corollary~\ref{cor:inscribed-quadrilateral} and \ref{ex:quadrilateral},
...
\parbf{\ref{ex:neutral-quadrilateral}.}
...
(Alternatively, you may apply \ref{ex:neutral-quadrilateral}.)
...
\parbf{\ref{ex:dual-desargues-construction}.}
...
\parbf{\ref{ex:revert}.} Suppose $p=(QR)$; denote by $q$ and $r$ the dual lines produced by the construction. Then, by Claim~\ref{clm:polar}, $P$ is the point of intersection of $q$ and $r$.
...
\parbf{\ref{ex:hex}.} Consider the bijection $P\z\leftrightarrow \hat P$ of the h-plane with absolute~$\Omega$.
...
\parbf{\ref{ex:ptolemy}.} Given a quadrilateral $ABCD$, we can choose the complex coordinates so that $A$ has complex coordinate $0$. 
...
\parbf{\ref{ex:equilateral triangle-verify}.}
...
\parbf{\ref{ex:ceva}.} Apply Claim~\ref{clm:area-ratio} to show that 
...
\parbf{\ref{ex:cross-ratio-area}.} To prove \textit{(\ref{ex:cross-ratio-area:a})}, apply Claim~\ref{clm:area-ratio} twice to the triangles $OL_iL_j$, $OL_jM_i$, and $OM_iM_j$.

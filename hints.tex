...
\parbf{Exercise~\ref{ex:isos-sides}.} To prove \textit{(a)}, apply SAS.
...
\parit{(b)} Note first that the angles $XPY$ and $X'PY'$ are vertical.
...
the left hand side is the coordinate of $Q'$ 
and the right hand side is the coordinate of the center of $\Gamma'$;
here $x$ and $y$ denote the coordinates of two points of intersection $\ell\cap\Gamma$.
...
Apply the main observation (\ref{thm:main-observ}\ref{h-reflect}).
...
\begin{lpic}[t(-4mm),b(-3mm),r(0mm),l(-0mm)]{pics/bisection-affine(1)}
\lbl[t]{2,6;$A$}
\lbl[t]{17,9;$B$}
\lbl[br]{8.5,10;$M$}
\end{lpic}
...
Fix a point~$X$.
By Axiom~\ref{def:proj-axioms:1}, any point $Y$ in the plane lies in a unique line passing thru~$X$.
...
\begin{lpic}[t(-4mm),b(-3mm),r(-2mm),l(0mm)]{pics/tessellation(0.8)}
\lbl[br]{10,38;$A$}
\lbl[bl]{33,38;$B$}
\lbl[t]{21,35;$C$}
\end{lpic}
...
is a motion of the Euclidean plane and the h-plane at the same time.
...
Note that solid square contains infinite number of mutually nonparallel segments.
...
According to Theorem~\ref{thm:area-of-triangle},
$\area\solidtriangle=\tfrac12\cdot a\cdot b$. %???LINE

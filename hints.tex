...
\parbf{Exercise~\ref{ex:dist-square}.} Check the triangle inequality for $A=0$, $B=1$ and $C=2$.
...
Since $2\cdot\alpha\equiv 2\cdot \beta$, Exercise~\ref{ex:2a=0} implies that
 $\alpha\equiv \beta$ or $\alpha\equiv \beta+\pi$.
In the later case the angles have opposite signs which is impossible.
...
\parbf{Exercises~\ref{ex:construction-perpendicular}--\ref{ex:tangent-circle}.}
...
%\parbf{Exercise~\ref{ex:parallel-central-symmetry}.}???

\parbf{Exercise~\ref{ex:smililar+parallel}.}
Since  $\ell\parallel (AB)$, it cannot cross $[AB]$.
...
By the transversal property (\ref{thm:parallel-2}), $\measuredangle BAP \equiv \pi -\measuredangle APQ$.
Since $P$ lies between $A$ and $C$, we get that 
$\measuredangle BAC=\measuredangle BAP$ and $\measuredangle APQ+\measuredangle QPC\z\equiv \pi$.
Hence $\measuredangle BAC= \measuredangle QPC$.

The same way we can prove that $\measuredangle ABC= \measuredangle PQC$.
By AA similarity condition, $\triangle ABC\z\sim\triangle PQC$, and therefore $\frac{PC}{AC}=\frac{QC}{BC}$.%???PIC
...
\parbf{Exercise~\ref{ex:line-coord}.} Let $M=(x,y)$. Note that $MA=MB$ if and only if
...
Then apply Corollary~\ref{cor:inscribed-quadrilateral}
to $\square AA'BB'$.
...
\parbf{Exercise~\ref{ex:tnagents+midpoint}.}
...
Apply Corollary~\ref{cor:inscribed-quadrilateral} twice for $\square ABYX$ and $\square ABY'X'$ and use the transversal property (\ref{thm:parallel-2}).

}

\parbf{Exercise~\ref{ex:a+b=c}.}
...
Prove and use that $\triangle AA'C\cong \triangle APB$. %???+PIC
...
\parbf{Exercise~\ref{ex:3x120}.} Guess a construction from the diagram.
...
\parbf{Advanced exercise~\ref{ex:3-construction-perp-clines}.}
...
\textit{(Note that in the Euclidean plane the statement follows from Corollary~\ref{cor:inscribed-quadrilateral} and Exercise~\ref{ex:quadrilateral},
...
\parbf{Exercise~\ref{ex:neutral-quadrilateral}.}
...
Mimic the proof of \ref{thm:3sum-a} for the shown fence made from copies of quadrilateral $ABCD$. 
...
Apply Exercise~\ref{ex:defect} to $\triangle ABC$ and $\triangle AXC$.


\parbf{Exercise~\ref{ex:neutral-rectangle}.}
Show that $B$ and $D$ lie on the opposite sides of $(AC)$.
Conclude that $\defect(\triangle ABC)+\defect(\triangle CDA)=0$.
...
(Alternatively, you may apply Exercise~\ref{ex:neutral-quadrilateral}.)
...
Spell the meaning of terms ``perpendicular'' and ``h-line'' and then apply Exercise~\ref{ex:construction-perp-clines}.
...
\begin{itemize}
 \item The function $x\mapsto e^x$ is increasing and the function $x\mapsto e^{-x}$ is decreasing.
 \item Without loss of generality, we may assume that $a\le b\le c$; in this case 
 \[-c\le a-b\le b-a\le c.\]
\end{itemize}
...
\parbf{Exercise~\ref{ex:afine-linear}.}
In each case check that the map is a bijection and apply Exercise~\ref{ex:line-coord}.
...
Apply a shear map that changes the angle between the lines (see Exercise~\ref{ex:afine-linear}\textit{\ref{ex:afine-linear:shear}}).
...
\parbf{Exercise~\ref{ex:parallelogram-rule}.}
Apply twice Exercise~\ref{ex:affine-par} and the parallelogram rule.
(Note that the case if $A$, $B$, $X$, and $Y$ are collinear requires extra work.)
 
\parbf{Exercise~\ref{ex:affine-continuous} and \ref{ex:affine-coordinates}.}
...
\parbf{Exercise~\ref{ex:inversive-angle}.}
...
\parbf{Exercise~\ref{ex:desargues-construction}.}
...
\parbf{Exercise~\ref{ex:dual-configurations}.} One may use the same diagram, but swap the labels $X\leftrightarrow U$, $p\leftrightarrow a$, and so on.
...
(It is a partial case of \index{Brianchon's theorem}\emph{Brianchon's theorem}.)

\parbf{Exercise~\ref{ex:dual-desargues-construction}.}
...
Try to construct point $Q'$ as in the dual Pappus' theorem (see the solution of Exercise~\ref{ex:dual-pappus}).

\parbf{Exercise~\ref{ex:revert}.} Suppose $p=(QR)$; denote by $q$ and $r$ the dual lines produced by the construction. Then, by Claim~\ref{clm:polar}, $P$ is the point of intersection of $q$ and $r$.
...
\parbf{Exercise~\ref{ex:proj-perp}.} 
...
Let $X$ denotes one of the points of intersections of $\Gamma$ and $[OP)$.
...
\parbf{Exercise~\ref{ex:5-gon},} \textit{(\ref{ex:5-gon:a})}.
...
\parbf{Exercise~\ref{ex:midpoint-proj}.}
Use that $\tfrac\pi4$ is an angle of an isosceles right triangle, Exercise~\ref{ex:construction-perpendicular}, and Proposition~\ref{prop:perp-perp}.
...
\parbf{Exercise~\ref{ex:subdivision}.} Fix a polygonal set $\mathcal{P}$.
...
\parbf{Exercise~\ref{ex:ceva}.} Apply Claim~\ref{clm:area-ratio} to show that 
...
\parbf{Exercise~\ref{ex:cross-ratio-area}.} To prove \textit{(\ref{ex:cross-ratio-area:a})}, apply Claim~\ref{clm:area-ratio} twice to the triangles $OL_iL_j$, $OL_jM_i$, and $OM_iM_j$.

...
Denote the parallelogram by $AXYZ$.
Suppose that the line $\ell$ intersects $(AX)$ and $(AZ)$
at $B$ and $C$ correspondingly.
To do \textit{(\ref{ex:desargues-construction:desargues})}, apply Desargues' theorem for $\triangle ABC$ and a point $O\in (AY)$. 
To do \textit{(\ref{ex:desargues-construction:pappus})}, apply Pappus' theorem for right triples of points on the lines $(CZ)$ and $(BX)$.
...
Construct a circle $\Gamma$, tangent to both lines, which crosses~$[OP)$;

...
then $x$ is called the \index{real!part}\emph{real part} and $y$ is the \index{imaginary!part}\emph{imaginary part} of the complex number~$z$.
...
Given a point $Z=(x,y)$, 
the complex number $z=x+ i\cdot y$ is referred to as the \index{complex coordinate}\emph{complex coordinate} of~$Z$.

If $O$, $E$, and $I$ are points in the plane 
...
If $y=0$, we say that the complex number $z$ is \index{real!complex number}\emph{real}; if $x=0$, we say that $z$ is \index{imaginary!number}\emph{imaginary}.
...
this inequality is also called the \index{triangle!inequality}\emph{triangle inequality}.
...
\eqlbl{eq:euler}$$
is called \index{Euler's formula}\emph{Euler's formula}.
...
If you are familiar with the power series for sine, cosine, and the exponential function, then the following might convince you that identity \ref{eq:euler} holds:
\begin{align*}
...
these are called the \index{polar!coordinates}\emph{polar coordinates} of the point~$Z$.

If $z$ is the complex coordinate of $Z$, then $\rho=|z|$. 
The value $\theta$ is called the \index{argument}\emph{argument} of $z$
(briefly, $\theta=\arg z$).
In this case, 
$$z=\rho\cdot e^{i\cdot\theta}=\rho\cdot(\cos\theta+i\cdot\sin\theta).\qquad\qquad\qquad\qquad $$
...
Let $a$, $b$, $c$, and $d$ be the complex coordinates of vertices of $\square ABCD$.
Then $\square ABCD$ is inscribed
if and only if the number
$$\frac{(a-b)\cdot(c-d)}{(a-d)\cdot(c-b)}$$
is real.
\end{thm}

The value $\frac{(a-b)\cdot(c-d)}{(a-d)\cdot(c-b)}$ is called the
\index{cross-ratio!complex cross-ratio}\emph{complex cross-ratio} of $a$, $c$, $b$, and $d$;
it will be denoted by \index{64@$(u,v;w,z)$}$(a,c;b,d)$.
...
Let $\mathbb{D}$ be the \index{disc}\emph{open unit disc} in the complex plane centered at~$0$;
...
\begin{thm}{Schwarz--Pick theorem}\index{Schwarz--Pick theorem}
...
Conclude that the inequality in the Schwarz--Pick theorem can be rewritten as

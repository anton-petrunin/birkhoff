...
\begin{figure}[h!]
\centering
\begin{lpic}[t(-8mm),b(0mm),r(0mm),l(0mm)]{pics/H2checkers_334(0.25)}
\end{lpic}     
\end{figure}
...
such that $P\ne Q$ and $R\ne Q$.
The \index{hyperbolic angle}\emph{hyperbolic angle $PQR$} (briefly $\angle_h PQR$)\index{11@$\angle_h$} is an ordered pair of h-half-lines $[QP)_h$ and $[QR)_h$.
...
\begin{enumerate}[(a)]
\item\label{h-line-to-hline} The h-line $(PQ)_h$, h-half-line $[PQ)_h$, and h-segment $[PQ]_h$ are transformed into $(P'Q')_h$, $[P'Q')_h$, and $[P'Q']_h$ correspondingly.
...
If $P\ne O$, by Lemma~\ref{lem:P-->O} and the main observation (\ref{thm:main-observ})
 there is an inversion that maps h-plane to itself and $O\mapsto P$.

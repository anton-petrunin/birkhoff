...
\section{The conformal disc model}
...
and call it the \index{absolute}\emph{absolute}.
...
The points in the h-plane will also be called \index{h-point}\emph{h-points}.
...
It is important to clarify at this point that, \textit{so far, an h-line $(PQ)_h$ is just a subset of the h-plane},
but soon we will introduce h-distance 
and show that $(PQ)_h$ is a line for the h-distance in the sense of Definition~\ref{def:line}.
...
\begin{thm}[!]{Classroom exercise}\label{ex:line/h-line}
...
After that we can use the axiomatic approach in the h-plane;
...
Consequently, if the system of axioms \ref{def:birkhoff-axioms:0}--\ref{def:birkhoff-axioms:3} and h-\ref{def:birkhoff-axioms:4} leads to a contradiction, then so does the system of axioms \ref{def:birkhoff-axioms:0}--\ref{def:birkhoff-axioms:4}.
...
Together with the lemma above, it implies that in any problem that is formulated entirely \textit{in h-terms} we can assume that a given h-point lies at the center of the absolute.
...
It follows that points inside the absolute remain inside after the mapping.
Whence $X\mapsto X'$ is a bijection from the h-plane to itself.
...
That is, if $P$ approaches the absolute in the Euclidean sense, it escapes to infinity in the h-sense.
...
Finally, notice that $\Delta_\rho'$ reflects to itself across $(OP)$;
...
\begin{thm}{Claim}
A subset of the h-plane is an h-line if and only if it is a line for the h-distance in the sense of Definition~\ref{def:line}.
\end{thm}
...
In this case, $\ell$ is the intersection of a diameter of the absolute and the h-plane.
...
We have shown that every h-line is a line for h-distance.
...
is continuous at every triple of points $(P,Q,R)$
...
&\measuredangle_h P' Q' R'&=\pm\measuredangle_h P Q R.
\end{align*}
...
It follows that this motion also gives a motion of the h-plane;
...
\item Draw three mutually parallel h-lines
such that every pair of these three lines lies on one side of the remaining h-line.

\item Draw h-lines $\ell$, $m$, and $n$ such that $\ell\parallel m$, $m\parallel n$, but $\ell\nparallel n$.
Conclude that parallelism is not an equivalence relation for h-lines.

\item Draw $h$-lines $k$, $\ell$, $m$, and $n$, such that $k\perp \ell$, $\ell\perp m$, $m\perp n$, and $k\parallel n$.
\end{enumerate}
\end{thm}

...
The portion of an h-line on one side from a point will be called a {}\emph{hyperbolic half-line} (or \index{h-half-line}\emph{h-half-line}).
...
If $\Gamma$ is the circline containing the h-line $(PQ)_h$, then the points ($A$ and $B$ on the diagram) where $\Gamma$ intersects the absolute are called 
...
and its center is the inverse of $P$ across the absolute. 
\end{thm}
...
Note that there is a point $T$ on the absolute such that $\angle OPT$ is right.
...
By AA, $\triangle TPP'\sim \triangle OTP'$ (the angle at $P'$ is shared, and two other angles are right). 
Therefore $OP'\cdot PP'=P'T^2$, and $O$ is the inversion of $P$ across~$\Gamma$.

It remains to calculate $P'T$.
Note that $OP'=\tfrac1{OP}$.
By the Pythagorean theorem $P'T^2=P'P^2+PT^2$ and $PT^2=1-PO^2$.
Since $P'P=OP'-OP$, the second statement follows.
\qeds

...
Namely, we construct  
so called \index{conformal disk model}\emph{conformal disk model} of hyperbolic plane.
...
Similarly a triangle in h-plane; that is, an ordered triple of h-points, say $(P,Q,R)$, will be denoted by \index{1gons@$\triangle_h$}$\triangle_h P Q R$.
...
at~$Q$.
Then the \index{angle measure!hyperbolic angle measure}\index{hyperbolic angle measure}\emph{hyperbolic angle measure} (or \index{h-angle measure}\emph{h-angle measure}) \index{1fig@$\measuredangle_h$}
...
we get that the model provides an example of absolute plane; 

...
\]
\[\tanh x\df \tfrac{\sinh x}{\cosh x}=\tfrac{e^{2\cdot x}-1}{e^{2\cdot x}+1}.
\]

The formula with hyperbolic cosine in following exercise will be used in the proof of the hyperbolic Pythagorean theorem (\ref{thm:pyth-h-poincare}).

\begin{thm}{Advanced exercise}\label{ex:cosh}
Let $P$ and $Q$ be two h-poins distinct from the center of absolute.
Denote by $P'$ and $Q'$ the inversions of $P$ and $Q$ in the absolute.
Show that 
\begin{enumerate}[(a)]
\item\label{ex:coshcosh} \[\cosh PQ_h=\frac{PQ\cdot  P'Q'+PQ'\cdot  P'Q}{PP'\cdot QQ'};\]
\item\label{ex:coshtanh} \[\tanh[\tfrac12\cdot PQ_h]=\sqrt{\frac{PQ\cdot  P'Q'}{PQ'\cdot P'Q}}.\]
\end{enumerate}

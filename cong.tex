...
Since $\triangle A' B' C'$ is nondegenerate, $(A' C')$ is distinct from~$(B' C')$.
...
let $f$ be a distance-preserving map from the plane to itself
such that 
$$f(A)=A,
\quad 
f(B)=B,
...
that is, $f(X)=X$ for every point $X$ in the plane.
\end{thm}

The statement in the exercise is called the \emph{three-nail lemma}, since three nails fix the plane in place. %(Unlike in Jerusalem, nobody gets hurt here.)
...
The idealized ruler can be used only to draw a line thru two given points.
...
say a tool that produces a midpoint for two given points,
...
to prove that the result of the construction satisfies the required conditions and also to determine when the construction is possible and how many solutions may exist.

...
\parit{``If'' part.}
Assume that \ref{eq:inscribed-angle} holds for some $P\z\notin \Gamma$.
Note that $\measuredangle X O Y\ne 0$. 
Therefore, $\measuredangle X P Y$ is neither $0$ nor $\pi$;
...
In the second case, 
applying the ``if'' part and the fact that  $P$, $X$, and $P'$ lie on one line (see Exercise~\ref{ex:ABCO-line}) we get that
\begin{align*}
...
Part \textit{(\ref{ex:inscribed-angle:power})} of the exercise makes it a useful tool for studying circles, but we are not going to consider it further in the book.)
...
Suppose $\Gamma$ is a circle with diameter~$[AB]$.
...
Let $\Gamma$ be a circle with center $O$, and let $\Delta$ be a circle with diameter $[OP]$.
...
\index{quadrangle!inscribed quadrangle}\emph{inscribed in a circle $\Gamma$}
...
and let $A'$ and $B'$ be footpoints of altitudes from $A$ and~$B$ respectively.
...
\section{The method of an additional circle}
...
is called the \textit{method of an additional circle}
...
\begin{thm}{Exercise}\label{ex:median-angle}
...
(this is the so-called \index{Simson line}\emph{Simson line} of $P$).
\end{thm}
...
The \index{circular arc}\emph{circular arc}~$ABC$ is the subset that includes the points $A$, $C$,
as well as all the points on $\Gamma$ that lie with $B$ on the same side of $(AC)$.

Points $A$ and $C$ are called 
\index{endpoint of arc}\emph{endpoints} of the circular arc $ABC$. 
There are precisely two circular arcs of $\Gamma$ with the given endpoints; they are \index{opposite arc}\emph{opposite} to each other.
...
A circular arc $ABC$ is defined if $\triangle ABC$ is nondegenerate.
If $\triangle ABC$ is degenerate, then arc $ABC$ is defined as a subset of the line bounded by $A$ and $C$ that contains $B$;
in this case we say that arc $ABC$ is \index{degenerate arc}\emph{degenerate}.
...
then the arc $AB'C$ is defined as the union of disjoint half-lines $[AX)$ and $[CY)$ in $(AC)$.
...
we also may use the term \index{circline!arc}\emph{circline arc} if we want to emphasize that the arc might be degenerate.
...
perform a ruler-and-compass construction of a point $Z$ such that
$\measuredangle AZB
= \measuredangle BZC
= \measuredangle CZA
\z=\pm\tfrac23\cdot\pi$.
...
If the arc is formed by the union of two half-lines $[AX)$ and $[BY)$ in $(AC)$,
...
Note that the tangent half-line to the arc $ABC$ at $A$ is uniquely defined.
Furthermore, there is a unique half-line $[AX)$ such that the equation in the proposition holds.
Therefore, it is sufficient to prove the ``only-if'' part.
...
If $[AX)$ is tangent to the arc $ABC$, then by \ref{thm:tangent-angle}
...
By the definition of a tangent half-line,
...
with given endpoints $A$ and $C$
that is tangent to a given half-line~$[AX)$.
\end{thm}

\begin{thm}{Exercise}\label{ex:tangent-lim}
Let $[AX)$ be a tangent half-line to an arc $ABC$.
Assume $Y$ is a point on the arc $ABC$ that is distinct from $A$.
Show that $\measuredangle XAY$ approaches $0$ as $AY$ approaches $0$.
...
Given two circular arcs $AB_1C$ and $AB_2C$, let $[AX_1)$ and $[AX_2)$ be half-lines tangent to the arcs $AB_1C$ and $AB_2C$ at $A$, and $[CY_1)$ and $[CY_2)$ be half-lines tangent to the arcs $AB_1C$ and $AB_2C$ at~$C$.
Show that

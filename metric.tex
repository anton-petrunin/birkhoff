...
will be denoted by \index{1fig@$\angle$}$\angle AOB$.
...
The angle measure of $\angle AOB$ is denoted by \index{1fig@$\measuredangle$}$\measuredangle AOB$;
...
With ``$\equiv$'', we can do addition, subtraction and multiplication by integer numbers without getting into trouble.
That is, if
$$\alpha\equiv\beta
\quad
\text{and}
...
say $A,B,C$,
is called a \index{triangle}\emph{triangle}\label{page:def:triangle}, and denoted by \index{1gons@$\triangle$}$\triangle A B C$.

...
\addtocontents{toc}{What is axiomatic approach?}
...
\addtocontents{toc}{What is model?}
...
\begin{itemize}
\item\index{Euclidean metric}\emph{Euclidean metric,} denoted as \index{$d_2$}$d_2$ and defined as \label{def:d_2}
$$d_2(A,B)=\sqrt{(x_A-x_B)^2+(y_A-y_B)^2}.$$
\item\index{Manhattan metric}\emph{Manhattan metric,} denoted as \index{$d_1$}$d_1$ and defined as 
$$d_1(A,B)=|x_A-x_B|+|y_A-y_B|.$$
\item\index{maximum metric}\emph{Maximum metric,} denoted as \index{$d_\infty$}$d_\infty$ and defined as 
...
Assume there is a line $\ell$ passing through
...
say $A,B,C$
is called \index{triangle}\emph{triangle} and denoted as $\triangle A B C$.
So the triangles $\triangle A B C$ and $\triangle A C B$ are considered as different.

Two triangles $\triangle A' B' C'$ and $\triangle A B C$ are called \index{triangle!congruent triangles}\index{congruent triangles}\emph{congruent}

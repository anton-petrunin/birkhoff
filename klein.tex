\chapter{Klein model}\label{chap:klein}
...
The picture on the right gives a new way to look at the hyperbolic plane, which is called \index{Klein model}\emph{Klein model}.
...
\lbl[t]{23,0;Poincar\'e model}
\lbl[t]{72,0;Klein model}
\end{lpic}
\end{center} 
...
\parbf{Angles.}
The angle measures in Klein model are very different from the Euclidean angles and it is hard to figure out by looking on the picture.\label{klein-angles}
...
where $\cosh$ denotes \index{$\cosh$}\index{hyperbolic cosine}\emph{hyperbolic cosine};
...
\begin{wrapfigure}[10]{o}{44mm}
\begin{lpic}[t(-7mm),b(-0mm),r(0mm),l(0mm)]{pics/boyai-constr(1)}
\lbl[bl]{21.5,21.5;$P$}
\lbl[tl]{21.5,12.5;$Q$}
\lbl[tr]{8.5,12.5;$R$}
\lbl[tr]{8.5,20;$S$}
